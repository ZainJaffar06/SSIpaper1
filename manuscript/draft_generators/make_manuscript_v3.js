...
const t2 = new Table({ width: { size: 9360, type: WidthType.DXA }, columnWidths: [2340, 2340, 2340, 2340], rows: [
  ...
  new TableRow({ children: [cell("ViT-B/16"), cell("0.725±0.049"), cell("0.677–0.774"), cell("−0.002 to −0.099 (0.10–0.93)")] }),
]});
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      new Paragraph({ alignment: AlignmentType.CENTER, spacing: { after: 140 }, children: [new TextRun({ text: "Paper 1 — Methods and Results (per review outline)", bold: true, size: 27 })] }),
      P([I("Completed per the review outline; all numbers trace to the released row-level out-of-fold predictions and the locked master fold map. One flagged deviation: Section 9 of Results uses corrected clinical/combined values (0.652→0.701; 0.734→0.817; combined-vs-clinical now P=0.013) after repair of empty intraoperative numeric covariates in the frame behind the outline's figures. All other numbers unchanged.")], { alignment: AlignmentType.LEFT }),
      ...
      P([T("Across three seeds per architecture on the identical locked folds (Table 2; Fig. 2a): EfficientNet-B0 0.755±0.037, ResNet18 0.731±0.052, ViT-B/16 0.725±0.049 (patient AUROC, mean±SD). All between-architecture paired deltas were non-significant (P=0.10–0.93), architecture rankings changed with seed, and the largest contrast observed anywhere was ResNet18 against itself at a different seed (−0.102; P=0.09), marginally exceeding the largest between-architecture difference (ViT-B/16 seed 7 vs reference, −0.099; P=0.10). The multi-seed training-size curve showed no upward trend: 0.718±0.016 at one-third of training patients, 0.746±0.057 at two-thirds, 0.755±0.037 at full size (Fig. 2b). "), B("Seed variance exceeds every architecture difference — capacity is not the bottleneck, and a bigger or different model does not help.")]),
      t2,
      ...
      P([T("The outcome was 30-day SSI by CDC/NHSN criteria"), T(" from structured chart review. Study photographs were not available to the adjudicating team and did not inform any diagnosis. "), B("Index images are therefore independent of the reference standard, pre-empting circularity between exposure and outcome — a property prior imaging studies have not clearly asserted.")]),
      ...
      P([T("Raw scores were used for discrimination and ranking; Platt scaling"), T(" (cross-fold) for probability and operating-point claims — recalibration is unstable at this event count and flexible (isotonic) calibration was degenerate; both facts are stated rather than hidden. Operating points were pre-specified (fixed specificity; high-sensitivity rule-out) with sensitivity, flag rate and clustered CIs. Decision-curve net benefit"), T(" was computed against treat-all/treat-none across the plausible threshold range, with CIs. "), B("Clinical use is framed as rule-out with an explicit flag rate; PPV is not headlined at 20 events.")]),
      H2("Comparative analyses"),
      P([T("Image versus clinical (intraoperative-only — the honest specification, since chart-review availability was outcome-correlated and covariate sets encoding ascertainment were excluded from inference) versus combined, on the locked cohort, with paired ΔAUROC/ΔAUPRC. The clinician panel rated identical images (pre-diagnosis-restricted contrast; Fleiss κ"), T(" for agreement; surgeon versus non-surgeon strata). "), B("The clinician comparison was pre-specified as exploratory — few positive patients fall in the shared-rated subset.")]),
      H2("Reproducibility"),
